...
impl Map {
    ...
    pub fn generate_borders(&mut self) {
        ...
        self.bottom.0 = (1..(x_max - 1))
            .map(|x| Vec2::new(x as i16, y_max as i16))
            .collect();
        self.left.0 = (1..(y_max - 1)).map(|y| Vec2::new(0, y as i16)).collect();
        self.right.0 = (1..(y_max - 1))
            .map(|y| Vec2::new(x_max as i16, y as i16))
            .collect();
        self.corners.0 = [(0, 0), (x_max, 0), (0, y_max), (x_max, y_max)]
            .into_iter()
            .map(|(x, y)| Vec2::new(x as i16, y as i16))
            .collect();
    }
}
